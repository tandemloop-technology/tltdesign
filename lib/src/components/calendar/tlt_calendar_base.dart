...
import 'tlt_calendar_core.dart';

class TltTableCalendarBase extends StatefulWidget {
  ...
  TltTableCalendarBase({
    ...
    this.onCalendarCreated,
        ...
        assert(
            isSameDate(focusedDay, firstDay) || focusedDay.isAfter(firstDay)),
        assert(isSameDate(focusedDay, lastDay) || focusedDay.isBefore(lastDay)),
        super(key: key);
...
class TltTableCalendarBaseState extends State<TltTableCalendarBase> {
  ...
  @override
  void didUpdateWidget(TltTableCalendarBase oldWidget) {
            ...
            child: TltCalendarCore(
              ...
              onPageChanged: (index, focusedMonth) {
                if (!_pageCallbackDisabled) {
                  if (!isSameDate(_focusedDay, focusedMonth)) {
  ...
  int _getDaysBefore(DateTime firstDay) {
    return (firstDay.weekday +
            7 -
            getWeekdayNumbers(widget.startingDayOfWeek)) %
        7;
  }

  int _getDaysAfter(DateTime lastDay) {
    int invertedStartingWeekday =
        8 - getWeekdayNumbers(widget.startingDayOfWeek);
