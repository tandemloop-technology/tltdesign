...
/// - see [TltDataTable.smRatio], [TltDataTable.lmRatio] and same properties on [PaginatedTltDataTable].
...
@immutable
class TltDataColumn extends DataColumn {
...
/// By using [TltDataColumn] instead of [DataColumn] it is possible to control
/// relative column sizes (setting them to S, M and L). [TltDataRow] provides
/// row-level tap event handlers.
class TltDataTable extends DataTable {
  TltDataTable({
    super.key,
  ...
  Widget build(BuildContext context) {
            ...
            for (int dataColumnIndex = 0;
              ...
              for (final DataRow row in rows) {
                ...
                var c = _buildDataCell(
                    ...
                    padding: padding,
                    specificRowHeight:
                        row is TltDataRow ? row.specificRowHeight : null,
                    label: cell.child,
                    numeric: column.numeric,
                    ...
                    onRowTap: row is TltDataRow ? row.onTap : null,
                    onRowDoubleTap: row is TltDataRow ? row.onDoubleTap : null,
                    onRowLongPress: row.onLongPress,
                    onRowSecondaryTap:
                        row is TltDataRow ? row.onSecondaryTap : null,
                    onRowSecondaryTapDown:
                        row is TltDataRow ? row.onSecondaryTapDown : null,
    ...
    //   debugPrint('TltDataTable built: ${sw.elapsedMilliseconds}ms');
    // }
    ...
    if (displayCheckboxColumn) {
      ...
      for (final DataRow row in rows) {
        var x = _buildCheckbox(
            ...
            onRowTap: () {
              if (row is TltDataRow && row.onTap != null) {
                row.onTap?.call();
              } else {
  ...
  List<double> _calculateDataColumnSizes(BoxConstraints constraints,
    ...
    var totalFixedWidth = columns.fold<double>(
        ...
        (previousValue, element) =>
            previousValue +
            (element is TltDataColumn && element.fixedWidth != null
                ? element.fixedWidth!
                : 0.0));
    ...
    final widths = List<double>.generate(columns.length, (i) {
      ...
      if (!(column is TltDataColumn && column.fixedWidth != null)) {
    ...
    for (var i = 0; i < widths.length; i++) {
      ...
      if (!(columns[i] is TltDataColumn &&
          (columns[i] as TltDataColumn).fixedWidth != null)) {
        widths[i] *= ratio;
      }
    }
